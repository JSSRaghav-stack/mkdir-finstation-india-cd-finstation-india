  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden" style={{ background: '#0a0a0f', color: '#e2e8f0' }}>
      ...
      >
        ...
        <div className="px-4 py-4" style={{ borderTop: '1px solid #1e1e2e' }}>
          <div className="text-xs" style={{ color: '#334155' }}>
            ...
            <div className="mt-1" style={{ color: '#1e3a5f' }}>v1.0 • NSE/BSE Data</div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <div>
            <h1 className="font-semibold text-sm" style={{ color: '#f1f5f9' }}>
              {NAV_ITEMS.find(n => n.id === activeSection)?.label}
            </h1>
            <p className="text-xs" style={{ color: '#475569' }}>
              {activeSection === 'dashboard' && 'Indian Equity Markets Overview'}
              {activeSection === 'company' && 'NSE/BSE Stock Analysis'}
              {activeSection === 'ai' && 'AI-Powered Equity Research Reports'}
              {activeSection === 'dcf' && 'Discounted Cash Flow Valuation Model'}
              {activeSection === 'lbo' && 'Leveraged Buyout Analysis'}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-xs px-2 py-1 rounded" style={{ background: 'rgba(34,197,94,0.1)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.2)' }}>
              ● NSE Live
            </div>
