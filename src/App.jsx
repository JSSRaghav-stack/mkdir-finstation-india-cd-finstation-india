...
export default function App() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden" style={{ background: '#0a0a0f', color: '#e2e8f0' }}>
      <aside className="flex flex-col flex-shrink-0 h-full" style={{ width: 220, background: '#0d0d15', borderRight: '1px solid #1e1e2e' }}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-3 flex-shrink-0"
          ...
          <div>
            <h1 className="font-semibold text-sm" style={{ color: '#f1f5f9' }}>{NAV_ITEMS.find(n => n.id===activeSection)?.label}</h1>
            <p className="text-xs" style={{ color: '#475569' }}>{SUBTITLES[activeSection]}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-xs px-2 py-1 rounded" style={{ background:'rgba(34,197,94,0.1)', color:'#22c55e', border:'1px solid rgba(34,197,94,0.2)' }}>● NSE Live</div>
            <div className="text-xs" style={{ color: '#475569' }}>{new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'})}</div>
          </div>
        </div>
